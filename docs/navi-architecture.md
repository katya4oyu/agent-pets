...
> \* **HermesAgent は先を見据えた仮称**であり、具体的な体験・連携 IF はまだ未設計。本書での扱いは
> 「将来こうした重い実行系を接続しうる」という**予告／拡張余地の確保**であって、近い実装対象ではない（§1 原則7）。
...
7. **E2E テスト可能性ゲート（実装は体験設計の後）**: 機能を実装するのは、その体験が設計され、
   **E2E テストで検証できる見通しが立ってから**。体験が未確定の構想（例: 後述の Outbound 操縦や
   HermesAgent 連携）は本書では **「予告／将来像」に留め、先回りして実装しない**。
   当面の安全な実装範囲は、既存仕様を変えずに既存テストで回帰を守れる**内部リファクタ（Phase 1）まで**。
...
- **Backend はプラグイン的に追加可能**: Claude Code / Codex / Copilot に加え、将来的には
  重い実行系も 1 つの Connector として登録できる構造にしておく（拡張余地の確保）。

> **本節（Outbound）は将来像であって近い実装対象ではない。** `HermesAgent` は仮称・体験未設計であり、
> Outbound 全体は体験設計と **E2E テスト計画が固まってから**着手する（§1 原則7）。現時点では
> 「Inbound のみで成立している現行機能を壊さず、Outbound を後付けできる余地を残す」ことだけを設計に織り込む。
