...
- 公開パッケージは **`packages/ui` の 1 つだけ**。**内部で codex-pet をコードとして分離**。
- **`examples/playground`**（独立アプリ）と **`app`**（Tauri）が `ui` を import。Cloudflare 公開対象は playground。
- 詳細・根拠・残課題は `docs/frontend-packaging.md`、TODO は `issues/a7f3d2-extract-frontend-ui-package.md`。
